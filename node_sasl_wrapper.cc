...
static sasl_callback_t *ExtractCallbacks(v8::Local<v8::Object> options, const char* _key) {
  ...
  std::unique_ptr<char[]> z_realm(new char[realm.size() + 1]);
  ...
  std::unique_ptr<char[]> z_user(new char[user.size() + 1]);
  ...
  std::unique_ptr<char[]> z_password(new char[password.size() + 1]);
  memcpy(z_password.get(), password.c_str(), password.size() + 1);
  std::unique_ptr<zsasl_secret_ctx> secret_ctx(new zsasl_secret_ctx());
  ...
  std::unique_ptr<sasl_callback_t[]> c_cbs(new sasl_callback_t[cbs.size()]);
